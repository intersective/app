...
describe('PusherService', async () => {
  ...
  describe('getChannels()', async () => {
    ...
    it('should ignore a notification-channel response from a previous experience', () => {
      ...
      apolloSpy.graphQLFetch.and.returnValue(response$);
    ...
    it('should ignore both channel responses when the experience changes mid-refresh', async () => {
      ...
      apolloSpy.graphQLFetch.and.returnValues(notificationResponse$, chatResponse$);
      storageSpy.getUser.and.returnValue({
        apikey: 'old-key',
        timelineId: 1,
      } as any);
  ...
  describe('initialise()', () => {
    beforeEach(() => {
      ...
      apolloSpy.graphQLFetch.and.callFake((query: string) => {
        return query.includes('notificationChannel')
          ? of(notificationRes)
          : of(pusherChatChannelRes);
      });
    });
  ...
  describe('normaliseTemplateValue() (private, tested via resolveUseTLS)', () => {
    it('returns empty string for unsubstituted template placeholders like <FOO>', () => {
      ...
      const originalUseTLS = (environment as any).pusherUseTLS;
  ...
  describe('resolvePusherPort() (private)', () => {
    it('returns undefined when pusherPort is not configured', () => {
      const originalPort = (environment as any).pusherPort;
