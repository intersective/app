...
import { ApolloConfig } from './apollo.module';
...
export class ApolloService {
  ...
  initiateCoreClient(): Apollo {
    if (this._hasInitiated(this.config.endpoint, ClientType.core)) {
    ...
    this._url.core = this.config.endpoint;
  ...
  private _hasInitiated(url: string, type: ClientType = ClientType.core): boolean {
    if (type === ClientType.core
      ...
      && this._url.chat === this.config.endpoint) {
        return true;
    }
    return false;
  }
  ...
  initiateChatClient() {
    if (this._hasInitiated(this.config.endpoint, ClientType.chat)) {
      return;
    }
