...
################################################
name: Deploy to p1-uk live environment
...
################################################
    env:
      ...
      STACK_NAME: p1-uk # Valid values are au,us,uk,p2,lf,nu,p1-sandbox,p1-stage,p2-sandbox,shared only
      ROOTSTACK: app-v2
      CFNS3BucketName: devops-cfn-templates
      PRIVATES3BucketName: devops-shared-private
      PUBLICZONENAME: uk.practera.com
      APPPUBLICZONENAME: uk.practera.app
      BUILD_CONFIG: custom
      STATUSREPORTS3Bucket: deployment-status.practera.com
      STATUS: DEPLOYED
      REQUESTOR: ${{ github.event.inputs.REQUESTOR }}
      REASON: ${{ github.event.inputs.REASON }}
      ENDPOINT: app.uk.practera.com
...
##########################################################
      - name: AWS Deploy Secret
        ...
        run: |
          ...
          echo "Enable P1 UK ACCOUNT CLI Login"
          P1_UK_AWS_ACCESS_KEY_ID=$(aws secretsmanager get-secret-value --secret-id $STACK_NAME-DEPLOY-$ENV| jq --raw-output '.SecretString' | jq -r .P1_UK_AWS_ACCESS_KEY_ID)
          P1_UK_AWS_SECRET_ACCESS_KEY=$(aws secretsmanager get-secret-value --secret-id $STACK_NAME-DEPLOY-$ENV| jq --raw-output '.SecretString' | jq -r .P1_UK_AWS_SECRET_ACCESS_KEY)
          echo "::add-mask::$P1_UK_AWS_ACCESS_KEY_ID"
          echo "::add-mask::$P1_UK_AWS_SECRET_ACCESS_KEY"
          echo "::set-output name=P1_UK_AWS_ACCESS_KEY_ID::$P1_UK_AWS_ACCESS_KEY_ID"
          echo "::set-output name=P1_UK_AWS_SECRET_ACCESS_KEY::$P1_UK_AWS_SECRET_ACCESS_KEY"
...
##########################################################
#  AWS P1 UK ACCOUNT STEPS BELOW
##########################################################
      - name: Configure AWS credentials from $STACK_NAME account in $REGION region
        uses: aws-actions/configure-aws-credentials@v1
        with:
          aws-access-key-id: ${{ steps.DeploySecret.outputs.P1_UK_AWS_ACCESS_KEY_ID }}
          aws-secret-access-key: ${{ steps.DeploySecret.outputs.P1_UK_AWS_SECRET_ACCESS_KEY }}
          aws-region: ${{env.REGION}}
        env:
          AWS_ACCESS_KEY_ID: ${{ steps.DeploySecret.outputs.P1_UK_AWS_ACCESS_KEY_ID }}
          AWS_SECRET_ACCESS_KEY: ${{ steps.DeploySecret.outputs.P1_UK_AWS_SECRET_ACCESS_KEY }}
...
# #########################################################  
      - name: Slack Notification
        ...
        with:
          ...
          job_name: deploy-p1-uk-live # Match the name above.
